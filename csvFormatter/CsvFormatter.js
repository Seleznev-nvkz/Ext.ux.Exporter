...
Ext.define("Ext.ux.exporter.csvFormatter.CsvFormatter", {
    ...
    format: function(store, config) {
        ...
        data = this.getHeaders() + this.lineSeparator + this.getRows(store);
        return Ext.ux.exporter.Base64.encode(data);
    },
        ...
        Ext.each(this.columns, function(col, iCol) {
            ...
            if(typeof name !== 'undefined') {
                if (Ext.isFunction(col.renderer)) {
                  value = col.renderer(record.get(name), {column: col}, record, index, iCol, record.store);
                } else {
